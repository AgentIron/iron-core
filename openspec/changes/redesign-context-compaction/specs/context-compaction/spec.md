...
### Requirement: Hidden runtime summarizer compaction
**Reason**: Model-driven `compress` replaces hidden runtime compaction so the active model authors the durable summary and compaction is visible through normal tool-call flow.
**Migration**: None. Existing testing sessions that depended on hidden `CompactedContext` compaction may break.

#### Scenario: Runtime runs separate compaction LLM call
- **WHEN** maintenance or hard-fit pressure is detected
- **THEN** the runtime no longer sends older history to a separate hidden summarizer request
